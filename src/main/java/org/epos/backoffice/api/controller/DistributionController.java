...
public class DistributionController extends MetadataAbstractController<Distribution> implements ApiDocTag{
    ...
    @ResponseBody
    @Operation(summary = "Get Distribution instances", description = "You can use this endpoint to retrieve all the Distribution instances (using \"all\" as path parameter instead of metaId) or a specific instance the endpoint will return only the instances which the user doing the request have access (more information into the BackOffice repository documentation)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "The Distribution instances are correctly retrieved", content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "400", description = "Bad request."),
            @ApiResponse(responseCode = "401", description = "Token is missing or invalid"),
