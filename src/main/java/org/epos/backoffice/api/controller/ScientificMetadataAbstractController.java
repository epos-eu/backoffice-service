...
public abstract class ScientificMetadataAbstractController<T extends EPOSDataModelEntity> extends BackofficeAbstractController<T> {
    ...
    protected ResponseEntity<?> deleteMethod(String instance_id) {
        ...
        if (list.isEmpty())
            return ResponseEntity
                    .status(404)
                    .body(new ApiResponseMessage(1, "No instance for this instanceId"));
        ...
        if (!computePermission.isAuthorized() || groupFilter.notOk())
            return ResponseEntity
                    .status(403)
                    .body(new ApiResponseMessage(1, "You don't have auth on the groups of this instance"));
        ...
        return ResponseEntity
                .status(200)
                .body(new ApiResponseMessage(4, "Instance deleted"));
    }
    ...
    protected ResponseEntity<?> postMethod(EPOSDataModelEntity body, boolean takeCareOfTheParent) {
        ...
        if (body.getMetaId() != null &&
            ...
            return ResponseEntity
                    .status(400)
                    .body(new ApiResponseMessage(1, "There is already a draft with the same metaId for this user"));
        ...
        if (Objects.nonNull(body.getInstanceChangedId()) && !body.getInstanceChangedId().isBlank()){
            ...
            if (retrieved.isEmpty())
                return ResponseEntity
                        .status(400)
                        .body(new ApiResponseMessage(1, "Non existing instance with that instaceChangedId"));
        ...
        if(Boolean.parseBoolean(body.getToBeDelete())){
            try {
                ...
                if(originalInstanceList.isEmpty()){
                    return ResponseEntity
                            .status(400)
                            .body(new ApiResponseMessage(1, "The instanceChangedId doesn't correspond to any existing instance"));
                }
                ...
                if (!Objects.equals(originalInstance.getState(),PUBLISHED)){
                    return ResponseEntity
                            .status(400)
                            .body(new ApiResponseMessage(1, "You can only delete in this way a published instance"));
                }
                ...
                return ResponseEntity
                        .status(400)
                        .body(new ApiResponseMessage(1, "Instance miss this field: " + e.getMessage()));
            }
        }
        ...
        try {
            ...
            return ResponseEntity.status(500).body(new ApiResponseMessage(1, "Something went wrong during the persisting of the new instance"));
        }
        //dbapi.rollbackTransaction();
    ...
    protected ResponseEntity<?> updateMethod(EPOSDataModelEntity body, boolean takeCareOfTheParent) {
        ...
        if (body.getInstanceId() == null)
            return ResponseEntity
                    .status(400)
                    .body(new ApiResponseMessage(1, "instanceId required."));

        List<T> retrieved = dbapi.retrieve(entityType, new DBAPIClient.GetQuery().state(DRAFT).instanceId(body.getInstanceId()));
        if (retrieved.isEmpty())
            return ResponseEntity
                    .status(400)
                    .body(new ApiResponseMessage(1, "There isn't a draft with this instanceId"));
        ...
        return ResponseEntity.status(201).body(
                new LinkedEntity().uid(body.getUid()).entityType(entityType.getSimpleName()).instanceId(instanceId).metaId(body.getMetaId())
        );
    }
    ...
    protected ResponseEntity<?> updateStateMethod(String instance_id, State newState, Boolean justThisOne) {
        ...
        if (instanceWithStateToBeUpdatedList.isEmpty()) {
            return ResponseEntity
                    .status(404)
                    .body(new ApiResponseMessage(1, "No instance with that instanceId"));
        }
        ...
        return ResponseEntity.status(201).body(
                new ApiResponseMessage(4, "correctly updated the state of the instance")
        );
